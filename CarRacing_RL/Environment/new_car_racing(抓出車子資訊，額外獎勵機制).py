...
def is_in_road(point, race_road):
    ...
    inside = False          # 紀錄車子是否在賽道內
    road_x1, road_y1 = race_road[0] # 紀錄第一個頂點
    for i in range(n + 1):      
        road_x2, road_y2 = race_road[i % n]
...
class CarRacing(gym.Env, EzPickle):
    ...
    def step(self, action: Union[np.ndarray, int]):
        ...
        if action is not None:
            ...
            action = [0.0, 0.0, 0.0] if self.continuous else 0
...
# '''   把速度、ABS感測器、方向盤位置、陀螺儀抓到info
        ...
        for poly, _ in self.road_poly:
            if is_in_road(car_pos, poly):
                ...
                center_point = poly_array.mean(axis=0) # 取得賽道中央座標
                center = np.linalg.norm(car_pos - center_point)
                break
        if center is None:
            center = 0.0
        track_half_width = TRACK_WIDTH  # 取得 賽道寬度/2
...
# '''
        ...
        if self.render_mode == "human":
            self.render()
        return self.state, step_reward, terminated, truncated, self.info
